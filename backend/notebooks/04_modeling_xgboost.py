...
# %% [markdown]
# ## Modelado — XGBoost (Telco Customer Churn)
#
# Entrena un `XGBClassifier` para predecir `churn_value` a partir del
...
# entrenamiento que el script reproducible `ml/train_xgboost.py` (fuente
# de verdad para generar el modelo en `ml/model_xgboost.pkl`), y añade
...
# ni el resto de la app — es un artefacto adicional.
...
# `total_charges`) sin transformar. XGBoost no requiere escalado.
#
# Se usa `scale_pos_weight` para compensar el desbalance de clases (~27%
# churn) y `GridSearchCV` (5-fold estratificado, optimizando ROC-AUC) para
# elegir `n_estimators`, `max_depth`, `learning_rate` y `subsample`.

# %% 4. Train + hyperparameter tuning
base_pipeline = build_pipeline()
spw = compute_scale_pos_weight(y_train)
base_pipeline.named_steps["model"].set_params(scale_pos_weight=spw)
...
ax.set_title("Curva ROC — XGBoost (test)")
ax.legend()
plt.tight_layout()
...
#
# Importancia nativa de XGBoost (`feature_importances_`) sobre las
...
#   `uv run python -m ml.train_xgboost` desde `backend/`:
#   - `ml/model_xgboost.pkl`
#   - `ml/metrics_xgboost.json`
#   - `ml/feature_importances_xgboost.csv`
# - El modelo usa `scale_pos_weight` para compensar el desbalance de
#   clases (~27% churn), calculado como ratio negativos/positivos.
# - Consistente con el EDA: antigüedad (`tenure_months`), tipo de contrato
#   y cargos mensuales están entre las variables con mayor importancia.
