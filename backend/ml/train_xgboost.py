"""Entrena un XGBClassifier para predecir churn (Telco Customer Churn).

Es un pipeline independiente y aditivo: no modifica `ml/predict.py` (inferencia
en producción) ni el resto de la app. Genera sus propios artefactos dentro de
`ml/`:

- `ml/model_xgboost.pkl`             -> pipeline sklearn (preprocesado + modelo) entrenado
- `ml/metrics_xgboost.json`           -> métricas de evaluación (holdout + cross-validation)
- `ml/feature_importances_xgboost.csv` -> importancia de features tras one-hot
...
METRICS_PATH = os.path.join(BASE_DIR, "metrics_xgboost.json")
FEATURE_IMPORTANCES_PATH = os.path.join(BASE_DIR, "feature_importances_xgboost.csv")
...
def build_pipeline() -> Pipeline:
    ...
    # scale_pos_weight compensa el desbalance de clases (~27% churn).
    # Se calcula como ratio de negativos sobre positivos en el conjunto de entrenamiento.
    model = XGBClassifier(
...
def main() -> dict:
    ...
    # Ajustar scale_pos_weight según el desbalance real del conjunto de entrenamiento
