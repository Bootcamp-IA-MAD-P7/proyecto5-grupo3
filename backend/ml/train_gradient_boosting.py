"""Entrena un GradientBoostingClassifier para predecir churn (Telco Customer Churn).

Es un pipeline independiente y aditivo: no modifica `ml/predict.py` ni el resto de la app.
Genera sus propios artefactos dentro de `ml/`:

- `ml/model_gradient_boosting.pkl`               -> pipeline sklearn entrenado (preprocesado + modelo)
- `ml/metrics_gradient_boosting.json`             -> métricas de evaluación en JSON
- `ml/feature_importances_gradient_boosting.csv`  -> importancia de features tras one-hot

Requiere `data/processed/telco_customer_churn_clean.csv`.
...
MODEL_PATH = os.path.join(BASE_DIR, "model_gradient_boosting.pkl")
METRICS_PATH = os.path.join(BASE_DIR, "metrics_gradient_boosting.json")
FEATURE_IMPORTANCES_PATH = os.path.join(BASE_DIR, "feature_importances_gradient_boosting.csv")
...
def load_data() -> pd.DataFrame:
    if not os.path.exists(PROCESSED_PATH):
        raise FileNotFoundError(
            f"No se encontró {PROCESSED_PATH}. Ejecuta antes el script o "
            "notebook de preprocesamiento para generar el dataset limpio."
        )
...
def tune_hyperparameters(pipeline: Pipeline, X_train: pd.DataFrame, y_train: pd.Series) -> GridSearchCV:
    # Optimización específica para Gradient Boosting
    param_grid = {
...
def main() -> dict:
    ...
    print(f"✅ Importancias guardadas en: {FEATURE_IMPORTANCES_PATH}")
